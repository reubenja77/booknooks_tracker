...
class BookUpdateDeleteTests(TestCase):
    def setUp(self):
        User = get_user_model()
        self.u1 = User.objects.create_user('reuben', 'reubenabrahams77@gmail.com', 'testpass123')
        ...
        self.b2 = Book.objects.create(title='Other User Book', author='B', owner=self.u2)
    ...
    def test_user_cannot_edit_others_book(self):
        ...
        resp = self.client.post(self.url_update_u2, {'title': 'Mercy', 'author': 'David Baldacci'})
    ...
    def test_user_cannot_delete_others_book(self):
        self.client.login(username='reuben', password='testpass123')
        resp = self.client.post(self.url_delete_u2,)
        self.assertEqual(resp.status_code, 404)
        self.assertTrue(Book.objects.filter(pk=self.b2.pk).exists())

    def test_user_can_update_own_book(self):
        self.client.login(username='reuben', password='testpass123')
        resp = self.client.post(self.url_update_u1, {'title': 'New Title', 'author': 'A'})
    ...
    def test_user_can_delete_own_book(self):
        self.client.login(username='reuben', password='testpass123')
        resp = self.client.post(self.url_delete_u1,)
